# -*- coding: utf-8 -*-
"""
Created on Thu Oct 26 15:13:09 2023
...
###############################################################################################################################
'Getting PV power gen data from the database'
print('##################################PV power generation data#####################################')
###############################################################################################################################
...
print('##########################################Forecasting##########################################')
###############################################################################################################################
# Name of PV power gen variable
var = 'pv_production'

# Merging PV power gen data with weather data
df_train = pd.merge(historical_PV, historical_weather, left_index=True, right_index=True)
df_test = pd.merge(historical_PV, df_forecast, left_index=True, right_index=True)
...
error2 = df_final[(df_final.Hour > 19) & (df_final[var] != 0)]
 
# Removing outliers of PV generation before 05:00 am and after 20:00 pm
df_final[var] = np.where(df_final.Hour < 5, 0, df_final[var])
df_final[var] = np.where(df_final.Hour > 19, 0, df_final[var])
...
heatmap.set_title('Features correlation with PV power generation', alpha= 0.75, weight= "bold", pad= 10)
...
plt.ylabel("Energy (W)", alpha= 0.75, weight= "bold")
...
plt.title("Train - Test split for "+ var, alpha= 0.75, weight= "bold", pad= 10, loc= "left")
plt.show()
...
plt.xlabel("Real generation (W)", alpha= 0.75, weight= "bold")
plt.ylabel("Predicted generation (W)", alpha= 0.75, weight= "bold")
plt.xticks(alpha= 0.75,weight= "bold",fontsize= 11)
plt.yticks(alpha= 0.75,weight= "bold", fontsize= 11)
plt.title("Correlation real vs predictions for "+ var, alpha= 0.75, weight= "bold", pad= 10, loc= "left")
plt.show()
...
plt.title("Real vs predicted for "+ var, alpha= 0.75, weight= "bold", pad= 10, loc= "left")
plt.show()
