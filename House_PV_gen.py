# -*- coding: utf-8 -*-
"""
Created on Thu Oct 26 17:21:28 2023
...
###############################################################################################################################
"""
Produces the house PV power generation forecasts for the next_hours defined

Args:
    data - dataframe containing the PV power generation and weather data (including training and test)
...
def forecasting(data, var, start_forecast, lat, lon):
    ...
    # Outliers (power generation before 05:00 am and after 20:00 pm)
    outliers = df_final[((df_final.Hour < 5) & (df_final[var] > 0)) | ((df_final.Hour > 19) & (df_final[var] > 0))]

    # Removing outliers of PV generation
    df_final[outliers][var] = 0 
